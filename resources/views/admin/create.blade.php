							...
							<form class="rounded-[2.5rem] border border-gray-100 bg-white p-10 shadow-xl shadow-gray-200/50"
								...
								<div class="mb-10">
									...
									<select
										...
										@foreach ($armada as $item)
											<option value="{{ $item->id }}">{{ $item->nama_brand }} ({{ $item->kode_identitas }})</option>
										@endforeach
									</select>
								</div>
								...
								<div class="mb-10">
									...
									<textarea
									 class="w-full rounded-2xl border border-gray-200 bg-gray-50 px-5 py-4 text-sm font-bold outline-none transition focus:ring-2 focus:ring-blue-500"
									 name="lokasi" rows="3" placeholder="Contoh: Bandara Soekarno-Hatta, Terminal 2" required></textarea>
								</div>
								...
								<div class="mb-10">
									...
									<input
										class="w-full rounded-2xl border border-gray-200 bg-gray-50 px-5 py-4 text-sm font-bold outline-none transition focus:ring-2 focus:ring-blue-500"
										name="stok" type="number" :class="stok > maxKapasitas ? 'border-red-500 ring-2 ring-red-200' : ''"
