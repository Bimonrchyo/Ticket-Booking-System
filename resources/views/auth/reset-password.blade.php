				...
				<form class="space-y-5" action="{{ route('password.reset') }}" method="POST">
					...
					<div>
						...
						<div class="relative">
							...
							<input
								...
								name="email" type="email" value="{{ old('email', $request->email) }}" required
					...
					<div>
						...
						<div class="relative">
							...
							<input
								class="w-full rounded-xl border border-gray-200 bg-gray-50 py-3 pl-11 pr-4 outline-none transition-all focus:bg-white focus:ring-2 focus:ring-blue-500"
								name="password" type="password" required placeholder="Masukkan password baru...">
						</div>
						@error('password')
							<p class="mt-1 text-sm text-red-600">{{ $message }}</p>
						@enderror
					...
					<div>
						...
						<div class="relative">
							...
							<input
								class="w-full rounded-xl border border-gray-200 bg-gray-50 py-3 pl-11 pr-4 outline-none transition-all focus:bg-white focus:ring-2 focus:ring-blue-500"
								name="password_confirmation" type="password" required placeholder="Konfirmasi password baru...">
						</div>
					</div>
